...
def test_all_by_field(preload, kwargs, expected_result, repo, runner, insert, request):
    repo = request.getfixturevalue(repo)

    assert repo.all_by_field(**kwargs, convert_to=TableEntity) == []

    for row in preload:
        insert("table", runner, row)

    assert repo.all_by_field(**kwargs, convert_to=TableEntity) == expected_result
